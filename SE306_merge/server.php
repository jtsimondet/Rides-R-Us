<?php 
	...
	if (isset($_POST['cust_reg'])) {
		...
		if (count($errors) == 0) {
			...
			$query = "INSERT INTO Customer (customerID, customerPWD, firstName, lastName, phoneNo, emailAddr, registerStatus) 
					  VALUES('$username', '$password', '$first_name', '$last_name', '$phone_number', '$email', 0)";
			if (mysqli_query($db, $query)) {
				...
				array_push($errors, "can't register, please contact admin");
			}
		}
	}
	...
	if (isset($_POST['driver_register'])) {
		...
		$last_name = mysqli_real_escape_string($db, $_POST['last_name']);
		$phone_number = mysqli_real_escape_string($db, $_POST['phone_number']);
		$payRate = mysqli_real_escape_string($db, $_POST['Pay_rate']);
		...
		if (count($errors) == 0) {
			...
			$query = "INSERT INTO Driver (driverID, driverPWD, firstName, lastName, phoneNo, payRate) 
					  VALUES('$username', '$password', '$first_name', '$last_name', '$phone_number', '$payRate')";
			if (mysqli_query($db, $query)) {
				$_SESSION['success'] = "Register Driver successfully";
				header('location: index.php');
			}else {
				array_push($errors, "can't register, Something wrong about this register form, please contact the development team");
			}
		}
	}
	...
	if (isset($_POST['remove_driver'])) {
		$driverID = mysqli_real_escape_string($db, $_POST['Driver_ID']);
		//error checking
		if (empty($driverID)){
			array_push($errors, "Please enter Driver ID");
		}
		//end error checking
		if (count($errors) == 0) {
			...
			$results = mysqli_query($db, $query);
			if (mysqli_query($db, $query)) {
				$_SESSION['success'] = "Remove driver $driverID successfully";
				header('location: remove_driver.php');
			}else {
				array_push($errors, "can't remove driver, please contact the network administrator");
			}
		}
	}
